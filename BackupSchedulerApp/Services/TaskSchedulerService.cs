        ...
        string logFilePath)
    {
        try
        {
            using (TaskService ts = new TaskService())
            {
                ...
                // Check if the task exists using the GetTasks method
                bool taskExists = folder.GetTasks().Any(task => task.Name.Equals(taskName, StringComparison.OrdinalIgnoreCase));
                if (taskExists)
                {

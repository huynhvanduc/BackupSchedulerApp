...
public static class BackupService
{
    ...
    /// <param name="logFilePath">The path to the log file for this specific worker run.</param>
    public static async Task PerformBackup(string sourcePath, string destinationPath, string logFilePath)
    {
        if (!Directory.Exists(sourcePath))
        {
            Logger.Log($"Lỗi: Thư mục nguồn không tồn tại: '{sourcePath}'", logFilePath);
            throw new DirectoryNotFoundException($"Thư mục nguồn không tồn tại: '{sourcePath}'");
        }

        string sourceFolderName = Path.GetFileName(sourcePath.TrimEnd(Path.DirectorySeparatorChar, Path.AltDirectorySeparatorChar));

        // Create a timestamped folder for the current backup
        string backupFolderName = string.Concat(sourceFolderName, "__", DateTime.Now.ToString("yyyy_MM_dd_HHmm"));
        ...
        catch (Exception ex)
        {
            ...
            throw new Exception($"Lỗi khi sao lưu: {ex.Message}", ex);
        }
    }
    ...
    /// </summary>
    private static async System.Threading.Tasks.Task CopyDirectory(string sourceDir, string destinationDir, string logFilePath)
    {
